...
Smith_Waterman::Smith_Waterman(int gap1, int gap2, int m, int nm){
    ...
    gap_penalty_ext = gap2;
}

Smith_Waterman::Smith_Waterman(int gap_open_penalty, int gap_extension_penalty, std::string blosum_path){
/**
 * Constructor.
 *
 * @param gap_open_penalty Gap open penalty. Default : 11.
 * @param gap_extension_penalty Gap expansion penalty. Default : 1.
 ...
 */
    ...
    gap_penalty_ext = gap_extension_penalty;
 ...
 */
{
    ...
    for(int i = 1; i < length1; i++)
    {
        for (int j = 1; j < length2; j++)
        {
            ...
            int b = maximum_on_rows[i] - gap_penalty_open - gap_penalty_ext*abs(j-maximum_on_rows_index[i]);

            /* c is gap penalty value on column j */

            int c = maximum_on_columns[j] - gap_penalty_open - gap_penalty_ext*abs(i-maximum_on_columns_index[j]);
            /* calculation of matrix element */
            matrix[i][j] = sw_max(a,b,c,0);
            if(matrix[i][j] >= maximum_on_rows[i] - gap_penalty_ext*abs(j-maximum_on_rows_index[i]))
            {
                // we made a new maximum we can memoize
                maximum_on_rows[i] = matrix[i][j];
                maximum_on_rows_index[i] = j;
            }
            if(matrix[i][j] > maximum_on_columns[j] - gap_penalty_ext*abs(i-maximum_on_columns_index[j]))
            {
                // we made a new maximum we can memoize
